...
function draw() {
	if( img_is_loaded ) {
		...
		ctx.fillText(top, width / 2, parseFloat(font_size.val())+parseFloat(padding_y.val()), width);
		ctx.fillText(bottom, width / 2, height - 10 - parseFloat(padding_y.val()), width);

		if( outline_size.val() > 0 ) {
			ctx.strokeStyle = color2.val();
			ctx.lineWidth = outline_size.val();
			ctx.strokeText(top, width / 2, parseFloat(font_size.val())+parseFloat(padding_y.val()), width);
			ctx.strokeText(bottom, width / 2, height - 10 - parseFloat(padding_y.val()), width);
		}
